...
avl_t *r_insert_node(avl_t **tree, avl_t *parent, avl_t **new, int nval)
{
	...
	{/* if ((*tree)->left == NULL)*/
		...
		if ((*tree)->left == NULL)
			return (NULL);/* return (*new = binary_tree_node(parent, nval));*/
	}
	else if ((*tree)->n < nval)
	{
	...
	palaver = binary_tree_balance(*tree);
	if (palaver > 1 && (*tree)->left->n > nval)
	{
		*tree = binary_tree_rotate_right(*tree);
	}
	else if (palaver > 1 && (*tree)->left->n < nval)
	{
		(*tree)->left = binary_tree_rotate_left((*tree)->left);
		*tree = binary_tree_rotate_right(*tree);
	}
	else if (palaver < -1 && (*tree)->right->n < nval)
	{
...
/*		if (*tree == NULL)*/
	...
	r_insert_node(tree, *tree, &newer, value);
	return (newer);
}
